...
namespace Whisper.Utils
{
    ...
    public class MicrophoneRecord : MonoBehaviour
    {
        ...
        private volatile bool _vadProcessing; // Track if VAD is being processed on thread pool
        ...
        private void Update()
        {
            ...
            var micPos = NormalizeMicPos(Microphone.GetPosition(RecordStartMicDevice));
        ...
        private void UpdateChunks(int micPos)
        {
            ...
            // Safety: limit iterations to prevent infinite loop in edge cases
            int loopCount = 0;
            const int maxIterations = 200;
            while (chunk > _chunksLength && loopCount < maxIterations)
            {
                loopCount++;
            ...
            {
                Debug.LogError($"[MicrophoneRecord] UpdateChunks exceeded max iterations. chunk={chunk}, _chunksLength={_chunksLength}, micPos={micPos}, ClipSamples={ClipSamples}");
            }
        }
        ...
        private void UpdateVad(int micPos)
        {
            ...
            // Skip if VAD is already being processed on thread pool
            if (_vadProcessing)
                return;
            ...
            // Get audio data on main thread (Unity API requirement)
            var data = GetMicBufferLast(micPos, vadContextSec);
            if (data.Length == 0)
                return;

            // Capture values for thread pool
            var clipFrequency = _clip.frequency;
            var capturedVadLastSec = vadLastSec;
            var capturedVadThd = vadThd;
            var capturedVadFreqThd = vadFreqThd;

            _vadProcessing = true;

            // Run SimpleVad on thread pool to avoid blocking main thread
            Task.Run(() =>
            {
                try
                {
                    var vad = AudioUtils.SimpleVad(data, clipFrequency, capturedVadLastSec, capturedVadThd, capturedVadFreqThd);
                ...
                {
                    LogUtils.Error($"VAD processing error: {ex.Message}");
                }
                finally
                {
        ...
        /// <summary>
        /// Normalize mic position to valid range.
        /// Unity can return values >= ClipSamples in edge cases.
        /// </summary>
        private int NormalizeMicPos(int micPos)
        {
            if (_clip != null && micPos >= ClipSamples)
                return micPos % ClipSamples;
            return micPos;
        }
