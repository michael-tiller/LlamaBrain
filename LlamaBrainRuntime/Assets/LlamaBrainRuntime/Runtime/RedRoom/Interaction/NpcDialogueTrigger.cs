...
namespace LlamaBrain.Runtime.RedRoom.Interaction
{
  public class NpcDialogueTrigger : MonoBehaviour, ITrigger
  {
    ...
    [Header("Events")]
    [SerializeField]
    private UnityEvent<string> onConversationTextGenerated;
    ...
    private async UniTask<string> GenerateConversationTextAsync(NpcFollowerExample npc)
    {
      ...
      try
      {
        string response = await agent.SayToNpc(promptText, cancellationTokenSource.Token);
        conversationText = response;
        Debug.Log($"[NpcDialogueTrigger] Dialogue generated. Length: {response?.Length ?? 0}");

        onConversationTextGenerated?.Invoke(response);
        return response;
      }
