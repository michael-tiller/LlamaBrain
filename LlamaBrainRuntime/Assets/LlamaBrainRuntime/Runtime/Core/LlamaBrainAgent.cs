...
namespace LlamaBrain.Runtime.Core
{
    ...
    public class LlamaBrainAgent : MonoBehaviour
    {
        ...
        public void Initialize(ApiClient client, PersonaMemoryStore memoryProvider)
        {
            ...
            Debug.Log("[LlamaBrainAgent] Starting initialization...");

            if (client == null)
            {
                Debug.LogError("[LlamaBrainAgent] Initialize failed: ApiClient is null");
                return;
            }

            if (memoryProvider == null)
            {
                Debug.LogError("[LlamaBrainAgent] Initialize failed: PersonaMemoryStore is null");
                return;
            }
            ...
            try
            {
                ...
                Debug.Log("[LlamaBrainAgent] Client and memory provider set successfully");
                ...
                if (PersonaConfig != null && runtimeProfile == null)
                {
                    ...
                    Debug.Log($"[LlamaBrainAgent] Converted PersonaConfig to profile: {runtimeProfile?.Name ?? "Unknown"}");
                }
                ...
                Debug.Log($"[LlamaBrainAgent] Dialogue session initialized with persona ID: {personaId}");

                Debug.Log($"[LlamaBrainAgent] Initialization complete. IsInitialized: {IsInitialized}");
                Debug.Log($"[LlamaBrainAgent] Final state - Client: {(client != null ? "Set" : "Null")}, MemoryProvider: {(memoryProvider != null ? "Set" : "Null")}, RuntimeProfile: {(runtimeProfile != null ? $"Set ({runtimeProfile.Name})" : "Null")}");
            }
            catch (System.Exception ex)
            {
                Debug.LogError($"[LlamaBrainAgent] Initialize failed with exception: {ex.Message}\nStackTrace: {ex.StackTrace}");
            }
        }
        ...
        public async UniTask<string> SendPlayerInputAsync(string playerName, string input)
        {
            ...
            if (client == null)
            {
                ...
                Debug.LogError($"[LlamaBrainAgent] {errorMsg}");
                return errorMsg;
            }
        ...
        public void AddMemory(string memoryEntry)
        {
            ...
            {
                Debug.LogWarning("[LlamaBrainAgent] Cannot add memory: Agent not initialized or runtime profile is null");
                return;
            }
                ...
                if (category != null && ShouldStoreMemory(category, kvp.Value))
                {
                    foreach (var extractedInfo in kvp.Value)
                    {
                        ...
                        if (!MemoryExists(category, extractedInfo))
                        {
                            ...
                            Debug.Log($"[LlamaBrainAgent] Extracted important memory using category '{category.DisplayName}': {extractedInfo}");
                        }
                    }
                }
            }
        ...
        public void ClearMemories()
        {
            ...
            Debug.Log($"[LlamaBrainAgent] Clearing all memories for {runtimeProfile.Name}");
        ...
        {
            Debug.Log($"[LlamaBrainAgent] Debug State:");
