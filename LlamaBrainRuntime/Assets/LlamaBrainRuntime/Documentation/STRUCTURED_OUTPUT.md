...
When building systems on LlamaBrain, design your APIs so that **structured output is the default** and free-form prose requires explicit justification. This "pit of success" pattern makes the right thing easy and the wrong thing possible but harder.

### The Problem

Without intentional API design, developers naturally reach for free-form generation:

```csharp
// The easy (but often wrong) path
var name = await agent.SendMessageAsync("Generate a merchant name");
// Returns: " Cogsworth\n[Output] Thaddeus Brass" (unpredictable format)
```

This leads to:
- Fragile regex parsing
- Format violations ("Output ONLY" instructions ignored)
- Hallucinated continuations
- Inconsistent game state

### The Solution

Design your wrapper API with two methods:

```csharp
// 1. Structured output: The obvious, easy path (schema extracted from type)
public async Task<T?> GenerateAsync<T>(string instruction, int seed, string? context = null)
    where T : class
{
    var schema = GetSchemaFromType<T>(); // Uses T.DefaultSchema via reflection
    return await SendStructuredInstructionAsync<T>(instruction, schema, context, seed);
}

// 2. Prose output: Requires explicit justification
public async Task<string?> GenerateProseAsync(
    string instruction,
    int seed,
    EProseReason reason,  // Forces intentional choice
    string? context = null)
{
    return await SendInstructionAsync(instruction, context, seed);
}
```

### Result Type Pattern

Define result types with embedded schemas:

```csharp
[Serializable]
public class NPCNameResult
{
    public const int SchemaVersion = 1;  // For future migration
    ...
    public const string DefaultSchema = @"{
      ...
      ""properties"": {
        ""name"": {
          ""type"": ""string"",
          ""description"": ""The NPC's full name (first and last name)"",
          ""minLength"": 3,
          ""maxLength"": 50
        }
      },
...
Define legitimate reasons for prose generation:

```csharp
public enum EProseReason
{
    // Descriptions - sensory, atmospheric
    ZoneDescription,
    RoomDescription,
    TavernAtmosphere,

    // Combat - dynamic, visceral
    CombatMessage,
    KillMessage,
    CriticalHitFlavor,
...
```csharp
// Good: Structured output for discrete data (names, stats, enums)
var npcName = await client.GenerateAsync<NPCNameResult>(
    "Generate a merchant name for gaslamp fantasy",
    seed: 12345);

// Good: Prose for creative content with explicit reason
var description = await client.GenerateProseAsync(
    "Describe the tavern atmosphere",
    seed: 12345,
    reason: EProseReason.TavernAtmosphere);

// Bad: Using prose for discrete data (no schema enforcement)
// var name = await client.GenerateProseAsync(...); // DON'T DO THIS
```

### Benefits

1. **Format compliance**: JSON schema enforces output structure at the LLM level
2. **Audit trail**: `EProseReason` documents why prose was chosen
3. **Reduced parsing**: No regex extraction needed for structured calls
4. **Determinism**: Same seed + schema = same output
5. **Friction is intentional**: If you can't name the reason, use structured

### When to Use Each

| Data Type | Use | Why |
|-----------|-----|-----|
| Names (NPC, item, location) | `GenerateAsync<T>()` | Discrete data, needs consistent format |
| Stats/numbers | `GenerateAsync<T>()` | Discrete data, needs type safety |
| Enum selections | `GenerateAsync<T>()` | Use `enum` constraint in schema |
| Descriptions | `GenerateProseAsync()` | Benefits from LLM creativity |
| Dialogue | `GenerateProseAsync()` | Needs character voice variety |
| Combat flavor | `GenerateProseAsync()` | Needs dramatic variety |
