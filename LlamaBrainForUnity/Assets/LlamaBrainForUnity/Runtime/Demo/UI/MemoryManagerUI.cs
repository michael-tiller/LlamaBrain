  ...
  public class MemoryManagerUI : MonoBehaviour
  {
    ...
    private void Start()
    {
      ...
      if (brainAgent == null)
      {
        ...
        if (brainAgent != null)
        {
          Debug.Log($"[MemoryManagerUI] Found UnityBrainAgent on same GameObject: {brainAgent.name}");
        }

        // If not found on same GameObject, try to find an initialized one in the scene
        if (brainAgent == null || !brainAgent.IsInitialized)
        {
          var allAgents = FindObjectsOfType<UnityBrainAgent>();
          Debug.Log($"[MemoryManagerUI] Found {allAgents.Length} UnityBrainAgent(s) in scene during Start");
      ...
      if (brainAgent == null)
      {
        Debug.LogWarning("[MemoryManagerUI] No UnityBrainAgent found in scene. UI will not function properly.");
      }
    ...
    private async UniTaskVoid WaitForInitializationAsync()
    {
      ...
      while (brainAgent == null || !brainAgent.IsInitialized)
      {
        ...
        var allAgents = FindObjectsOfType<UnityBrainAgent>();
        Debug.Log($"[MemoryManagerUI] Found {allAgents.Length} UnityBrainAgent(s) in scene");

        // Look for an initialized agent first
        UnityBrainAgent initializedAgent = null;
        foreach (var agent in allAgents)
        {
          Debug.Log($"[MemoryManagerUI] Agent: {agent.name}, IsInitialized: {agent.IsInitialized}");
          if (agent.IsInitialized)
    ...
    /// <summary>
    /// Force find and connect to a UnityBrainAgent
    /// </summary>
    [ContextMenu("Force Find Agent")]
    public void ForceFindAgent()
    {
      brainAgent = FindObjectOfType<UnityBrainAgent>();
      if (brainAgent != null)
      {
        Debug.Log($"[MemoryManagerUI] Force found UnityBrainAgent: {brainAgent.name}. IsInitialized: {brainAgent.IsInitialized}");
        brainAgent.DebugAgentState();
        UpdateUI();
      }
      else
      {
        Debug.LogWarning("[MemoryManagerUI] No UnityBrainAgent found in scene");
      }
    }
